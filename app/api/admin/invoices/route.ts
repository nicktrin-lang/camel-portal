...
import { generateCommissionInvoice } from "@/lib/portal/invoiceGenerator";
...
export async function GET(req: Request) {
  try {
    ...
    let query = db
      ...
      .select("id, invoice_number, partner_user_id, partner_id, period_start, period_end, currency, subtotal, booking_count, storage_path, issued_at, emailed_at, status")
    ...
    const invoices = await Promise.all(
      (data || []).map(async (inv: any) => {
        ...
        const periodMonth = inv.period_start ? inv.period_start.slice(0, 7) : null;
        return {
          ...inv,

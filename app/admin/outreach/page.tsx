    ...
    try {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          const json = await res.json();
          if (res.ok && json.prospect) { newProspects.push(json.prospect); added++; }
          else skipped++;
        } catch { skipped++; }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          >
            {batchSending
              ? batchProgress ? `Sending ${batchProgress.done}/${batchProgress.total}…` : "Sending…"
      ...
      {/* CSV result */}
      {csvResult && (
        <div className={["border px-4 py-3 text-sm font-semibold", csvResult.startsWith("✅") ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-700"].join(" ")}>
          {csvResult}
        </div>
      )}
        ...
        <div className="border border-black/10 bg-white p-6">
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input required value={form.company_name} onChange={e => setForm(f => ({ ...f, company_name: e.target.value }))} className="w-full border border-black/20 bg-[#f0f0f0] px-3 py-2 text-sm font-semibold text-black outline-none focus:border-black" placeholder="Hertz Spain SL" />
            </div>
            <div>
              <label className="block text-xs font-black uppercase tracking-widest text-black/50 mb-1">Contact Name</label>
              <input value={form.contact_name} onChange={e => setForm(f => ({ ...f, contact_name: e.target.value }))} className="w-full border border-black/20 bg-[#f0f0f0] px-3 py-2 text-sm font-semibold text-black outline-none focus:border-black" placeholder="Juan García" />
            </div>
            <div>
              <label className="block text-xs font-black uppercase tracking-widest text-black/50 mb-1">Email *</label>
              <input required type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="w-full border border-black/20 bg-[#f0f0f0] px-3 py-2 text-sm font-semibold text-black outline-none focus:border-black" placeholder="info@example.com" />
            </div>
            <div>
              <label className="block text-xs font-black uppercase tracking-widest text-black/50 mb-1">City</label>
              <input value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))} className="w-full border border-black/20 bg-[#f0f0f0] px-3 py-2 text-sm font-semibold text-black outline-none focus:border-black" placeholder="Madrid" />
            </div>
            <div>
              <label className="block text-xs font-black uppercase tracking-widest text-black/50 mb-1">Country</label>
              <select value={form.country} onChange={e => setForm(f => ({ ...f, country: e.target.value }))} className="w-full border border-black/20 bg-[#f0f0f0] px-3 py-2 text-sm font-semibold text-black outline-none focus:border-black">
            ...
            <div>
              ...
              <input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} className="w-full border border-black/20 bg-[#f0f0f0] px-3 py-2 text-sm font-semibold text-black outline-none focus:border-black" placeholder="Serves Malaga airport" />
            </div>
            {addError && <div className="sm:col-span-2 bg-red-50 border border-red-200 px-3 py-2 text-xs font-semibold text-red-700">{addError}</div>}
            <div className="sm:col-span-2 flex gap-3">
              <button type="submit" disabled={addLoading} className="bg-[#ff7a00] px-6 py-2 text-sm font-black text-white hover:opacity-90 disabled:opacity-40">{addLoading ? "Adding…" : "Add Prospect"}</button>
              <button type="button" onClick={() => setShowAdd(false)} className="px-6 py-2 text-sm font-black text-black/50 hover:text-black">Cancel</button>
            </div>
          </form>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10">
                <tr className="border-b border-black/10 bg-[#f0f0f0]">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-black uppercase tracking-widest text-black/50">Opened</th>
                  <th className="px-4 py-3 text-left text-xs font-black uppercase tracking-widest text-black/50">Clicked</th>
                ...
                {filtered.map(p => (
                  <tr key={p.id} className={["hover:bg-[#f8f8f8] transition-colors", p.unsubscribed ? "opacity-50" : ""].join(" ")}>
                    ...
                    <td className="px-4 py-3">
                      <select value={p.status} onChange={e => handleStatusChange(p.id, e.target.value as Status)} className={["text-xs font-black px-2 py-1 border-0 outline-none cursor-pointer", STATUS_STYLES[p.status]].join(" ")}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2 items-center flex-wrap">
                        {p.status === "pending" && !p.unsubscribed && (
                          <button onClick={() => handleSend(p.id)} disabled={sending === p.id || batchSending || remaining === 0} className="bg-[#ff7a00] px-3 py-1 text-xs font-black text-white hover:opacity-90 disabled:opacity-40 transition-opacity whitespace-nowrap">
                            {sending === p.id ? "Sending…" : "Send"}
                          </button>
                        )}
                        {!p.unsubscribed && (
                          <button onClick={() => handleSend(p.id, true)} disabled={sending === p.id || batchSending || remaining === 0} className="border border-black/20 px-3 py-1 text-xs font-black text-black/50 hover:border-black hover:text-black disabled:opacity-40 transition-all whitespace-nowrap">
                        ...
                        <button onClick={() => handleDelete(p.id, p.company_name)} disabled={deleting === p.id} className="text-xs font-black text-black/30 hover:text-red-600 transition-colors disabled:opacity-40">

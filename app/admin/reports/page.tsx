...
function insurancePill(driver: boolean, customer: boolean) {
  ...
  return (
    <span className={`inline-flex border px-2 py-0.5 text-xs font-black uppercase tracking-widest ${
      both?"border-green-200 bg-green-50 text-green-700":driver?"border-[#ff7a00]/30 bg-[#ff7a00]/10 text-[#ff7a00]":"border-amber-200 bg-amber-50 text-amber-700"
    }`}>
...
function calcPayout(b: BookingRow): { hire:number; rate:number; commAmt:number; payout:number; fuelRefund:number } {
  ...
  const basePayout = b.partner_payout_amount!=null ? Number(b.partner_payout_amount) : Math.max(0,hire-commAmt);
            ...
            {visible.map((b,i)=>{
              ...
              return (
                <tr key={b.id} className={`hover:bg-[#f0f0f0] ${isCancelled?"bg-red-50/50":i%2===0?"bg-white":"bg-[#fafafa]"}`}>
                  ...
                  <td className={`px-4 py-3 font-black ${isCancelled&&b.refund_status==="full"?"text-red-600":"text-green-700"}`}>
...
export default function AdminReportsPage() {
  ...
  const partnerMap = new Map<string,{name:string;bookings:number;revenue:number;commission:number;payout:number;completed:number;cancelled:number}>();
  for (const b of filteredBookings) {
    ...
    const cur = partnerMap.get(pid)||{ name, bookings:0, revenue:0, commission:0, payout:0, completed:0, cancelled:0 };
    cur.bookings++;
    if (!isCancelled) { cur.revenue+=isFinite(Number(b.amount??0))?Number(b.amount??0):0; cur.commission+=commAmt; cur.payout+=payout; }
  ...
  function exportExcel() {
    ...
    const summaryHeaders = [
      ...
      "Camel Commission","Partner Payout Total","Fuel Deposits","Fuel Charges Billed","Fuel Refunds Issued",
    ];
    const summaryRows = (["EUR","GBP","USD"] as Currency[]).map(curr=>{
      const t=revenuesByCurrency[curr];
      return [`${curr} ${CURRENCY_META[curr].symbol}`,t.count,t.completed,t.cancelled,t.total,t.carHire,t.commissionTotal,t.partnerPayoutTotal,t.fuelDeposit,t.fuelCharge,t.fuelRefund];
    });
    const partnerHeaders = ["Partner","Total Bookings","Completed","Cancelled","Total Revenue","Camel Commission","Partner Payout"];
    const partnerRows = partnerBreakdown.map(p=>[p.name,p.bookings,p.completed,p.cancelled,p.revenue,p.commission,p.payout]);
    const allHeaders = [
  ...
  if (loading) return (
    <div className="border border-black/10 bg-white p-8">
      <p className="text-black/50">Loading reports…</p>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-black/10 bg-white p-6 md:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-black/50">Full network-wide reconciliation including cancellations, commission, fuel and multi-currency revenue.</p>
          </div>
      ...
      <div className="border border-black/10 bg-white p-6 md:p-8">
        ...
        <p className="text-sm text-black/50 mb-4">Revenue, commission and payout performance by partner.</p>
        <div className="overflow-x-auto border border-black/10">
          <table className="min-w-full text-sm">
            <thead className="bg-black text-white">
              <tr>
                {["Partner","Bookings","Completed","Cancelled","Total Revenue","Camel Commission","Partner Payout"].map(h=>(
            ...
            <tbody className="divide-y divide-black/5">
              {partnerBreakdown.length===0?(
                <tr><td colSpan={7} className="px-4 py-4 text-black/50">No partner data.</td></tr>

...
// Convert from stored currency to EUR (so we can then go to any target)
...
// Partner always sees stored currency primary, EUR secondary (unless already EUR)
...
function BookingSummaryCard({ booking, rates, isLive }: {
  ...
  // Rate badge — only show EUR/GBP rate if booking involves those currencies
  const rateBadge = stored === "USD"
    ? `1€ = ${fmtCurr(rates.GBP, "GBP")} · 1€ = ${fmtCurr(rates.USD, "USD")}`
    : stored === "GBP"
    ? `1€ = ${fmtCurr(rates.GBP, "GBP")}`
    : `1€ = ${fmtCurr(rates.GBP, "GBP")}`;
  ...
  return (
    <div className="rounded-3xl border border-[#003768]/20 bg-[#003768] p-8 text-white shadow-[0_18px_45px_rgba(0,0,0,0.18)]">
      ...
      {/* Rate badge */}
      <div className={`mt-5 inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold ${
...
export default function PartnerBookingDetailPage() {
  ...
  // Rate badge text — currency-aware
        ...
        <div className="rounded-3xl border border-black/5 bg-white p-8 shadow-[0_18px_45px_rgba(0,0,0,0.08)]">
          ...
          <div className="mt-6 space-y-3 text-slate-700">
            ...
            <p>
              ...
              <Amt amount={bk.amount} stored={stored} rates={rates} />
            </p>

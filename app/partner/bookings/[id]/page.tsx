...
function PaymentFeesCard({ payment, bidCurrency, booking }: { payment: PaymentData; bidCurrency: Currency; booking: BookingRow }) {
  ...
  // Stripe fee converted to bid currency
  const feeInBid = (() => {
    ...
    <div className="border border-black/10 bg-[#f8f8f8] p-6">
      ...
      {hasCurrConv && payment.exchange_rate != null && (
        <div className="mt-4 border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-800">
          ⚠ Customer paid in {chargeCurr}. Stripe applied a conversion rate of {payment.exchange_rate.toFixed(5)} ({chargeCurr} → {bidCurrency}). The Stripe fee includes both the processing fee and the currency conversion fee. See your <a href="/partner/terms" className="underline">partner terms</a> for details.
        </div>
      )}
      {!hasCurrConv && (
